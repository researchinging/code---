...
//合并两个堆
void FibHeapUnion(FibHeap * heap1,FibHeap * heap2);
...
void FibHeapUnion(FibHeap * heap1,FibHeap * heap2){
       ...
       if(heap2->min==NULL) return;
        ...
        if(heap1->min == NULL){
            ...
            FibNode* H1_next = heap1->min->right;
            FibNode* H2_pre = heap2->min->left;

            heap1->min->right = heap2->min;
            H2_pre->right = H1_next;
...
void FibHeapConsolidate(FibHeap * heap) {
    ...
    while (NULL != heap->min) {
        ...
        while (NULL != *(heap->cons + d)) {
            ...
            *(heap->cons + d) = NULL;
            d++;//合并过程会产生d+1的新度的堆，故要再次检查忧心的相同的度吗？
        }
        *(heap->cons + d) = x;
    }
